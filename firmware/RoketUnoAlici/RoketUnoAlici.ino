...
struct FlightConfig {
  ...
  static constexpr unsigned long BURNOUT_CONFIRM_TIME = 200; // Normal burnout doğrulama süresi (ms)
  ...
  // Pyro Ayarları
  static constexpr unsigned long PYRO_PULSE_MS = 1000;     // Normal pyro pulse süresi (ms)
  static constexpr unsigned long SUT_PYRO_PULSE_MS = 2000; // SUT pyro pulse süresi (ms)
...
void setup() {
  ...
  // İlk sensör okumalarını yap (ham veri)
  ...
  if (!initialPitchSet && gEuler.orientation.y != 0.0f) {
    initialPitch = gEuler.orientation.y;
...
void readSensors() {
  ...
  if (!bmp.performReading()) {
    ...
    if (rawAlt > -1000.0f && rawAlt < FlightConfig::VALID_ALT_MAX) {
      ...
      fAlt = kalmanAlt; // artık uçuş algoritması filtrelenmiş veriyi kullanıyor
...
void updateFlightData() {
  ...
  if (fAlt > 0 && fAlt < FlightConfig::VALID_ALT_MAX) {
    if (fAlt > maxAlt) {
      maxAlt = fAlt;
      #ifdef DEBUG_FLIGHT_ALGO
      if (sut_active) {
...
bool detectLaunch() {
  ...
  // Ground level'dan minimum yükselme kontrolü
  float altGain = fAlt - groundLevel;
  if (!launch && altGain > FlightConfig::LAUNCH_ALT_MIN && 
      altGain < 100.0f && fAlt < FlightConfig::VALID_ALT_MAX) {
    launch = true;
    ...
    #ifdef DEBUG_FLIGHT_ALGO
    if (sut_active) {
      Serial.print("SUT - Fırlatma algılandı! Ground: ");
    } else {
      Serial.print("Fırlatma algılandı! Ground: ");
    }
    Serial.print(groundLevel, 1);
...
bool detectBurnout() {
  ...
  if (++burnoutDebugCounter % 5 == 0) { // Daha sık - her 500ms
    ...
    Serial.print(", Ratio: ");
    Serial.print(maxAccelTotal * (sut_active ? FlightConfig::SUT_BURNOUT_ACCEL_RATIO : FlightConfig::BURNOUT_ACCEL_RATIO), 1);
    Serial.print(", Thresh: ");
    Serial.print(sut_active ? FlightConfig::SUT_BURNOUT_ACCEL_THRESHOLD : FlightConfig::BURNOUT_ACCEL_THRESHOLD, 1);
    Serial.print(", Cand: ");
  ...
  // Burnout koşullarını kontrol et (SUT ve Normal mod ayrı parametreler)
  float accelThreshold, accelRatio;
  unsigned long confirmTime;
  
  if (sut_active) {
    // SUT modu - daha hızlı algılama
    accelThreshold = FlightConfig::SUT_BURNOUT_ACCEL_THRESHOLD;
    accelRatio = FlightConfig::SUT_BURNOUT_ACCEL_RATIO;
    confirmTime = FlightConfig::SUT_BURNOUT_CONFIRM_TIME;
  } else {
    // Normal mod - daha stabil algılama
    accelThreshold = FlightConfig::BURNOUT_ACCEL_THRESHOLD;
    accelRatio = FlightConfig::BURNOUT_ACCEL_RATIO;
    confirmTime = FlightConfig::BURNOUT_CONFIRM_TIME;
  }
  ...
  // SUT modunda daha agresif: VE mantığı kullan
  // Normal modda daha stabil: VEYA mantığı kullan
  bool burnoutCondition;
  if (sut_active) {
    burnoutCondition = condition1 && (maxAccelTotal > 5.0f); // SUT: Ratio VE minimum max ivme kontrolü
  } else {
    burnoutCondition = condition1 || condition2; // Normal: VEYA mantığı
  }
  ...
  if (burnoutCondition != lastBurnoutCondition) {
    ...
    if (sut_active) {
      Serial.print(" (SUT-VE mantığı, MaxAcc:");
      Serial.print(maxAccelTotal, 1);
      Serial.println(")");
    } else {
      Serial.println(" (Normal-VEYA mantığı)");
    }
  ...
  if (burnoutCondition) {
    if (!burnoutCandidate) {
      ...
      if (millis() - burnoutCandidateTime >= confirmTime) {
        ...
        #ifdef DEBUG_FLIGHT_ALGO
        if (sut_active) {
          Serial.print("SUT - Burnout onaylandı! Final İvme: ");
        } else {
...
bool detectDescent() {
  ...
  if (!desc && maxAlt > 0 && fAlt > 20.0f && 
      fAlt < FlightConfig::VALID_ALT_MAX && (maxAlt - fAlt) >= FlightConfig::DESC_DROP_M) {
    desc = true;
...
void controlPyroChannels() {
  ...
  // Yerel rakım offset'i ile paraşüt yükseklikleri
  float localOffset = groundLevel;
  if (!sep1 && desc && angleFlag && fAlt > (FlightConfig::DROGUE_ALT_MIN + localOffset)) {
    if (!pyro1Fired) {
      ...
      if (sut_active) {
        Serial.println("SUT - MOSFET Pyro 1 tetiklendi! (Drogue, 2s)");
      } else {
        Serial.println("MOSFET Pyro 1 tetiklendi! (Drogue)");
      }
    }
  }
  ...
  if (!sep2 && desc && fAlt <= (FlightConfig::MAIN_ALT_MAX + localOffset) && fAlt >= (FlightConfig::MAIN_ALT_MIN + localOffset)) {
    if (!pyro2Fired) {
      ...
      if (sut_active) {
        Serial.print("SUT - MOSFET Pyro 2 tetiklendi! (Main, 2s) İrtifa: ");
      } else {
        Serial.print("MOSFET Pyro 2 tetiklendi! (Main) İrtifa: ");
      }
      Serial.print(fAlt);
      Serial.println("m");
    }
...
void updateStatusBits() {
  ...
  bool mini_irtifa_esigi = (fAlt > FlightConfig::MIN_ALT_THRESHOLD && 
                            fAlt < FlightConfig::VALID_ALT_MAX);
...
void receiveSutPacket() {
  ...
  if (recvChecksum == calcChecksum) {
    ...
    if (irtifa != 0.0f) {
      fAlt = irtifa; // SUT irtifa verisi
    }
    
    // SUT açı verisi için de Kalman filtresi uygula (başlangıç açısından sapma olarak)
    float rawSutPitch = gEuler.orientation.y - initialPitch;
